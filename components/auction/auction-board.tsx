      ...
      channel = supabase
        ...
        .on(
          ...
          async (payload) => {
            ...
            setItems((current) =>
              current.map((item) => (item.id === payload.new.id ? transformedItem : item)),
            )
          },
        )
        .subscribe()
        ...
        <div className="relative max-w-3xl mx-auto">
          <div className="relative group">
            ...
            <Input
              ...
              className="pl-12 sm:pl-14 pr-4 py-5 sm:py-6 md:py-7 text-base sm:text-lg bg-neutral-800 border-2 border-neutral-700 focus:border-[#C9A961] focus:ring-2 focus:ring-[#C9A961]/20 rounded-xl shadow-md focus:shadow-lg transition-all text-white placeholder:text-neutral-500"
            />
          </div>
          {searchQuery && (
      ...
      <div className="space-y-8 sm:space-y-10 md:space-y-12">
          ...
          <section className="space-y-4 sm:space-y-5 md:space-y-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="relative" ref={dropdownRef}>
                <button
                  onClick={() => setOpenDropdown(openDropdown === "sorted" ? null : "sorted")}
                ...
                {openDropdown === "sorted" && (
                  <div className="absolute right-0 mt-2 w-48 bg-neutral-800 border-2 border-neutral-700 rounded-lg shadow-xl z-30">
                    <div className="py-1">
                      {[
                        ...
                        >
                          ...
                          <span className={sortFilter !== option.value ? "ml-6" : ""}>{option.label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {sortedItems.map((item) => (
                <AuctionItem key={item.id} item={item} isEnded={isEnded} />
              ))}
            </div>
          </section>
        ) : (
        ...
        {sortedItems.length === 0 && (
          <div className="text-center py-12">
            <p className="text-lg text-neutral-400">No honors found matching "{searchQuery}"</p>
          </div>
        )}
      </div>
    </div>
