...
import { HolidayTemplateManager } from './holiday-template-manager';
...
export function AuctionSetup({ onSuccess }: AuctionSetupProps) {
  ...
  const loadHolidayTemplate = (templateName: string) => {
    const template = templates.find(t => t.name === templateName);
    
    if (template) {
      setHolidayName(template.name);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-6">
        ...
        <div className="mb-6 p-4 border border-neutral-700 rounded-lg bg-neutral-800/50">
          ...
          {/* Holiday Templates */}
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2 text-neutral-400">Load from Template</label>
            <div className="flex gap-2 flex-wrap">
              {templates.map((template) => (
                <button
                  key={template.id}
                  onClick={() => loadHolidayTemplate(template.name)}
                  className={`px-3 py-1.5 rounded text-sm transition-colors ${
                    holidayName === template.name
